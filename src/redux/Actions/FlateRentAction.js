...
//
//
// https://erpcons.vitasoftsolutions.com/flate-rent/
//
export const createFlateRent = createAsyncThunk(
  "createFlateRent",
  async (payload) => {
...
//
//
export const fetchFlateRentList = createAsyncThunk(
  "fetchFlateRentList",
  async (page, { getState }) => {
    // Get the JWT token from session storage
    const token = sessionStorage.getItem("jwt_token");

    const { perPage } = getState().flateRentReducer;
    ...
    const response = await axios.get(
      `${base_url}/flate-rent/?limit=${perPage}&offset=${
        (page - 1) * perPage
      }`,
      {
        headers,
...
//
//
export const fetchFlateRentAllList = createAsyncThunk(
...
//
//
export const fetchFlateRent = createAsyncThunk("fetchFlateRent", async (id) => {
  ...
  // Make the Axios GET request with the headers
  const response = await axios.get(`${base_url}/flate-rent/${id}/`, {
    headers,
  });
...
//
//
export const deleteFlateRent = createAsyncThunk(
  "deleteFlateRent",
  async (payload) => {
    // Get the JWT token from session storage
    const token = sessionStorage.getItem("jwt_token");
...
//
//
export const updateFlateRent = createAsyncThunk(
  "updateFlateRent",
  async (payload) => {
    console.log(payload, "payload");
    // Get the JWT token from session storage
    ...
    const response = await axios.patch(
      `${base_url}/flate-rent/${payload.id}/`,
      payload.data,
      { headers }
    );
...
//
//
export const searchFlateRent = createAsyncThunk(
  "searchFlateRent",
  async (firstName) => {
    try {
      // Get the JWT token from session storage
      ...
      let apiUrl = `${base_url}/flate-rent/`;
...
//
//
export const sortByDateFlateRent = createAsyncThunk(
  "sortByDateFlateRent",
  async (date) => {
    try {
      // Get the JWT token from session storage
...
//
//
export const sortByAZFlateRent = createAsyncThunk(
  "sortByAZFlateRent",
  async (sortOrder) => {
    try {
      // Get the JWT token from session storage
